...
export function CreateGiftForm() {
  ...
  // Map react-hook-form field names to the IDs generated by the Input component (label-based)
  const fieldIdMap: Record<string, string> = {
    recipientName: "recipient's-name",
    recipientPhone: "recipient's-phone",
    amountNgn: "gift-amount-(₦)",
    unlockAt: "unlockAt",
    notifyAt: "notifyAt",
    message: "message",
  };
  ...
  // Focus error summary when validation errors appear after submit attempt
  useEffect(() => {
    ...
    <>
      <form className={styles.form} onSubmit={handleSubmit(onFormSubmit as Parameters<typeof handleSubmit>[0])} noValidate>
        ...
        {errorEntries.length > 0 && (
          ...
          >
            <p className={styles.errorSummaryTitle}>Please fix the following errors:</p>
            <ul className={styles.errorSummaryList}>
              {errorEntries.map(([field, err]) => (
                <li key={field}>
                  <a href={`#${fieldIdMap[field] ?? field}`}>{(err as { message: string }).message}</a>
                </li>
              ))}
            </ul>
          </div>
